...
it("Add new claim", (done) => {
  api.post('/claims/add').send({
    ...
    res.body.user.should.equal('Dhroov7');
    res.body.repo.should.equal('boss');
    res.body.bounty.should.equal(500);
    res.body.status.should.equal('claimed');
    res.body.issueUrl.should.equal('https://github.com/coding-blocks/boss/issues/59');
    res.body.pullUrl.should.equal('https://github.com/coding-blocks/boss/pull/60');
    done()
  });
});
...
it("Get claims", (done) => {
  api.get('/claims').end((err, res) => {
    ...
    res.body[1].rows[0].user.should.equal('Dhroov7');
    res.body[1].rows[0].repo.should.equal('boss');
    res.body[1].rows[0].bounty.should.equal(500);
    res.body[1].rows[0].status.should.equal('claimed');
    res.body[1].rows[0].issueUrl.should.equal('https://github.com/coding-blocks/boss/issues/59');
    res.body[1].rows[0].pullUrl.should.equal('https://github.com/coding-blocks/boss/pull/60');
    done()
  });
});
